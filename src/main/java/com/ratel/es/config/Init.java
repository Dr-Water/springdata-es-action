...
/**
 * @业务描述： 用于初始化数据执行一次即可，执行第一遍以后注释掉 @Component注解（不注释也行）
 * @package_name： com.ratel.es.config
 * @project_name： springdata-es-action
 * @author： ratelfu@qq.com
 * @create_time： 2020-01-12 15:37
...
public class Init {
   ...
   // @PostConstruct
    public void init() {
        ...
        logger.info("开始初始化数据,");
        //只初始化一次
        Iterable<Poem> poems = poemRepository.findAll();
        //如果没有从es取出数据，则说明是第一次启动，否则直接return避免不必要的创建，浪费时间
        if (poems.iterator().hasNext()) {
            logger.info("已经存在数据，不需要初始化");
            return;
        }
        for (int i = 0; i < 40; i++) {
            Poem poem = new Poem();
            poem.setTitle(getTitle().get(i));
            poem.setContent(getContent().get(i));
            poem.setWeight(i);
            poem.setUserId(i % 10);
            poemRepository.save(poem);
        }
        logger.info("初始化数据完成，耗时"+(System.currentTimeMillis()-startTime)+"ms");
    }
